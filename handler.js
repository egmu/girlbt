    ...
    try {
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (xp > 200)
                    this.sendButton(m.chat, `[❗] *Sepertinya Anda Bermain Curang, Menggunakan Calculator*`, author, null, [['BUY LIMIT', '/buy'], ['TOPUP LIMIT', '/topup']] , m)
                else
                    m.exp += xp
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.sendButton(m.chat, bottime, `[❗] *Limit Anda Habis, Beberapa Fitur Tidak Bisa Di Akses, Limit akan di reset setiap 24 Jam, Besok balik ke sini lagi ya 😊*`, thumbbc, [['BUY LIMIT', '/buy'], ['AMBIL MONEY', '/claim']] , fpay)
